...
2. `<STDIN>`で入力した好きな数値や文字列を、for文で配列`@array3`に設定し、for文で出力してください。
...
- `sort`のみ, あるいは`sort { $a cmp $b } @array`と書くと, 「文字列」としてソートします
    ...
    my @lang = qw(perl php ruby python java go);
    my @sorted = sort @lang; #=> go java perl php python ruby

    my @num = ( 5, 200, 40, 3, 1 );
    my @sorted = sort @num; #=> (1, 200, 3, 40, 5)
